...
Given a policy $\pi=[n_1,\dots,n_L]$ with integer threshold $n_k$ for the $k$-th segment in Algorithm \ref{alg:nested_wait}, we formulate a linear system analogous to Equation \eqref{eq:nested_wait_thresholds}; real-valued designs are rounded to the nearest feasible integer thresholds with only \(O(1)\) changes in the asymptotic bounds:
...
We validate the segment-wise design numerically using a real-data workload derived from the \texttt{lmsys-chat-1m} dataset~\cite{zheng2023lmsys} (Section~\ref{sec:experiments}). Decode lengths range up to 500 tokens in this workload, and the distribution of arrival rates, shown in Figure~\ref{fig:lambda_list}, is proportional to normalized frequencies from real user traffic.
