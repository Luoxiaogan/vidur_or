...
More generally, iteration time can exhibit piecewise linear behavior across different operating regimes. For example, \cite{li2025throughput} models iteration time as $\tau(b') = c + a \cdot \max\{0, b' - b_0\}$, where iteration time is constant for small batch sizes and grows linearly beyond a threshold $b_0$. This captures the transition between compute-bound and memory-bound regimes: when batches are small, GPU cores are the bottleneck; when batches are large, memory bandwidth dominates. Our algorithms and analysis extend naturally to such piecewise linear models. The threshold mechanism in WAIT (Section~\ref{sec:wait}) and Nested WAIT (Section~\ref{sec:nested_wait}) controls the number of prompts in each decode stage, ensuring the system operates within safe memory limits regardless of whether iteration time is strictly linear or piecewise linear. The key insight—balancing arrivals and completions to approach the fluid equilibrium—remains valid as long as iteration time scales predictably with batch composition. We adopt the simplified linear model~\eqref{eq:time_consump} for tractability in our theoretical analysis, while noting that the empirical validation in Figure~\ref{fig:inference time} confirms this approximation is accurate in the decode-dominated regime where our algorithms operate.
...
\begin{enumerate}
    ...
    \item \textbf{Admit all 6 for prefill}: After prefill, these 6 prompts advance to decode. The 3 existing decode prompts complete. System now has 6 prompts in decode, occupying $6 \times 2 = 12$ units---\textbf{memory is full}.
...
This example reveals the \textbf{cascading eviction problem} our algorithms target. Even with $C \geq M^*$ (the equilibrium memory from our fluid analysis, Section~\ref{sec:fluid}), poor admission control triggers this cascade, making the system unstable. The eviction-restart cycle can persist indefinitely, degrading throughput significantly (Example~\ref{ex:fcfs_cascade} quantifies this for FCFS, showing 12-25\% throughput loss).
\end{example}
...
\label{eq:optimization}
    ...
    & \ex{}{\TTFT^{(T,\pi)}} \le F^T, \\
    & \sum_{i \in \mathcal{G}^t} (l_{j(i)} + s^t_i) \le C, \quad \forall t.
    \end{aligned}
\end{equation}
