...
Real-world LLM inference systems often encounter prompts whose types—and consequently their output lengths—are unknown upon arrival. To address this challenge, we propose the \textit{Nested Waiting for Accumulated Inference Threshold} (Nested WAIT) algorithm to manage stochastic arrivals with unknown types.

\subsection{Algorithm Overview}

To begin with, note that adapting algorithms for known lengths by assuming worst-case lengths is inefficient. For example, with two prompt types (input length 1, arrival rates equal), one with output length 1 and another 10,000, assuming 10,000 for all yields memory utilization as low as \( 1/5000 \). Therefore, the Nested WAIT algorithm introduce a nested structure to manage prompts with unknown output lengths. The mechanism tries to distinguish different types of arrivals by iterating step by step, rather than assuming or guessing this information at the beginning. 
...
We evaluate Nested WAIT’s performance under conventional heavy-traffic regimes, where the arrival rate and service rates are scaled by $(\lambda,d_0,d_1)\to(\zeta\lambda,\zeta^{-1}d_0,\zeta^{-1}d_1)$ for $\zeta\to\infty$. Unknown prompt types increase complexity, as variable output lengths affect memory and processing times. The primary metric is average throughput, \( \throughput^{(\zeta, \pi)} \), where \( \zeta \) is the scaling parameter and \( \pi \) is the Nested WAIT policy. We also provide theoretical results for average latency \( \Latency^{(\zeta, \pi)} \) and time-to-first-token \( \TTFT^{(\zeta, \pi)} \).
...
Intuitively, with \( C = M^* \), ignorance of output lengths risks under-utilization or overflow after each iteration. Overflow forces restarts, reducing efficiency. Both scenarios yield a throughput gap of \( \Omega(1 / (\zeta T)) \), accumulating to \( \Omega(1) \) over time. 
...
\begin{equation}
    ...
    &\frac{n_{i+1}}{n_i} > p_i,\forall i \in \{1, \dots, m-1\},
    \end{aligned}
\end{equation}
where $p_i = (\sum_{j=i+1}^m\lambda_j)/(\sum_{j=i}^m\lambda_j)<1.$ Additionally, we define the memory usage when running iteration with exactly \( n_i \) prompts per stage in segment \( i \), for all \( i \in [1, \dots, m] \) as:
\begin{equation}
    \label{eq:nested_wait_memory}
\begin{aligned}
        M^\pi &= \sum_{j=1}^m n_j \big(l + \tfrac{1}{2}L_j'\big) \Delta l_j', \\
    &\text{where } L_j' = \sum_{k=1}^j l_k',\ \Delta l_j' = l_j' - l_{j-1}',\ l_0' = 0. 
\end{aligned}
\end{equation}   
 % $\arc{Give definition and bound S by $S\le T / \min\{\Delta T_{batch}$. Give definition of }
...
The following theoretical result demonstrates Nested WAIT’s asymptotic optimality under heavy-traffic conditions with unknown prompt types, given thresholds \( \pi = [n_1, \dots, n_m] \) satisfying Equation \eqref{eq:nested_wait_thresholds}.
...
\begin{aligned}
    &M^{(\zeta,\pi)}\\
\geq &M^\pi + \sum_{j=2}^m (l + l_{j-1}') \bigg(n_j + \theta_j^{-1} \ln\Big( \frac{m \zeta S}{\delta} \Big)\bigg) \\
=& O\bigg( 2M^\pi + \sum_{j=1}^m (l + l_{j-1}') \theta_j^{-1}\ln\Big( \frac{m \zeta S}{\delta} \Big) \bigg),
\end{aligned}
\end{equation}
where \( \theta_j \) is with lower bound $8(n_j-n_{j-1}p_j)/n_j.$ Then:
$$
    \begin{aligned}
