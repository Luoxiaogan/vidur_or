...
\begin{enumerate}
    \item \emph{Base memory $M^\pi$.} This is the equilibrium memory from the fluid model, the memory needed when exactly $n_k$ prompts occupy each stage in segment $k$. It represents the steady-state memory consumption.
    ...
    \begin{itemize}
        ...
        \item $(l + l_{k-1}')$: Memory consumption of prompts at the boundary between segments $k-1$ and $k$.
    \end{itemize}
\end{enumerate}
...
\textbf{Safety buffer overhead:} The memory requirement in~\eqref{eq:nested_wait_memory_total} is typically dominated by the base term $M^\pi$. The safety buffer (second term) grows only logarithmically in the time horizon and is modest in practice. Section~\ref{sec:experiments} provides numerical validation.
