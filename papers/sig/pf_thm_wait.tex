...
\paragraph{Case 1: Zero Drift ($\Delta T_{[1,\ldots,m]} = n_j/\lambda_j$).} When $\lambda_j\cdot \Delta T_{[1,\ldots,m]} = n_j$, we have $\ex{}{Y^b_{(j)}}=0$ (zero drift). By Lemma~\ref{lemma::throughput gap, single}:
