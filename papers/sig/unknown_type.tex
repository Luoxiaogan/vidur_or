...
WAIT achieves near-optimal throughput when prompt types are known at arrival. In practice, however, output lengths are unknown and reveal gradually during decode. The \textit{Nested WAIT} algorithm exploits this: \textbf{prompts classify themselves on-the-fly} as they decode, with short prompts completing early and longer prompts advancing to later segments. \textbf{No prediction is needed}.
...
Nested WAIT uses \textbf{nested thresholds} to resolve the dilemma in Example~\ref{ex:unknown_output}: segment $k$ handles prompts from stage $l_{k-1}' + 1$ to $l_k'$ (with $l_0' = 0$), each with threshold $n_k$. Short prompts complete in early segments without being blocked by long prompts; each segment operates at high memory utilization for its remaining types.
...
This lower bound shows that uncertainty has a cost: we need more memory than the fluid equilibrium to hedge against unknown output lengths. Theorem~\ref{thm:nested_wait_heavy_traffic} shows that only a logarithmic amount of extra memory suffices.
...
\begin{aligned}
        M^\pi &= \sum_{k=1}^m n_k \left(l + \tfrac{1}{2}L_k'\right) \Delta l_k', \\
...
\begin{aligned}
    M^{(\zeta,\pi)} &\geq M^\pi + \sum_{k=2}^m (l + l_{k-1}') n_k \\
    &\quad + \sum_{k=2}^m (l + l_{k-1}') \theta_k^{-1} \ln\left( \frac{m \zeta B}{\delta} \right) \\
    &= O\left( 2M^\pi + \sum_{k=1}^m (l + l_{k-1}') \theta_k^{-1}\ln\left( \frac{m \zeta B}{\delta} \right) \right).
\end{aligned}
\end{equation}
...
This coupling explains the \textbf{safety buffer} in Equation~\eqref{eq:nested_wait_memory_total}: when segment $k$ has fewer than $n_k$ prompts, queueing at boundaries introduces stochastic fluctuations. Doob's inequality bounds maximum queue length with high probability as $O(\ln(T/\delta))$—this logarithmic overhead is the safety buffer. Despite unknown output lengths, thresholds keep segments near equilibrium, preventing cascading evictions (Example~\ref{ex:fcfs_cascade}). Full proof in Appendix~\ref{appendix::proof of nested_wait_heavy_traffic}.
...
The safety buffer is \emph{logarithmic} in $T$ and $1/\delta$, making it modest compared to base memory $M^\pi$ (Proposition~\ref{prop:unknown_lower_bound} shows some extra memory is necessary; Theorem~\ref{thm:nested_wait_heavy_traffic} shows $O(\ln(T/\delta))$ suffices). Performance depends primarily on arrival rates and output lengths, not $m$. In practice, we use a \textbf{segment-wise variant} grouping $m$ types into $L \leq m$ segments (Section~\ref{sec:extension}), addressing robustness to varying distributions and infeasibility of per-type thresholds for long decode lengths. Section~\ref{sec:experiments} provides numerical validation.
