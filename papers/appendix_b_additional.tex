...
This appendix reports supplemental robustness and implementation-fidelity analyses for Section~\ref{sec:experiments}. Section~\ref{app:no_wait} isolates the role of the accumulation requirement by comparing WAIT with a threshold-only variant. Section~\ref{app:gpu_validation} validates the simulator against direct GPU measurements and reports supplemental physical-GPU comparisons on single-type and real-data workloads. Section~\ref{app:multi_seed} examines repeated-run variability near the observed transition from near-overloaded to overloaded operation, and Section~\ref{app:pd_disagg} evaluates the policy in a prefill-decode-disaggregated deployment.
