...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{fig_WAIT.png}
...
\Delta T_{[1,\dots,m]}(n_1,\dots,n_m):=d_0 + d_1 \sum_{j=1}^m n_j (l_j' + 1) \left( l_j + \frac{l_j'}{2} \right) \leq \frac{n_j}{\lambda_j}, \quad M^{\pi} = \sum_{j=1}^m n_j (l_j' + 1) \left( l_j + \frac{l_j'}{2} \right) \geq M^*,
\end{equation}
...
\label{thm:wait_heavy_traffic}
    ...
    \begin{align*}
        &\throughput^*-\ex{}{\throughput^{(\zeta,\pi)}} = O\prn{(\zeta T)^{-1/2}},\\ 
        &\ex{}{\Latency^{(\zeta,\pi)}} = O\prn{(\zeta T)^{1/2}},\quad\ex{}{\TTFT^{(\zeta,\pi)}}=O\prn{(\zeta T)^{1/2}}.
    \end{align*}
    Moreover, when we have $\Delta T_{[1,\dots,m]}<n_j/\lambda_j,\forall j\in[m]$ where $\Delta T_{[1,\dots,m]}$ is defined in \eqref{eq:wait_thresholds}, it holds that
        \begin{align*}
        &\throughput^*-\ex{}{\throughput^{(\zeta,\pi)}} = O\prn{(\zeta T)^{-1}},\\ 
        &\ex{}{\Latency^{(\zeta,\pi)}} = O\prn{1},\quad\ex{}{\TTFT^{(\zeta,\pi)}}=O\prn{1}.
    \end{align*}
\end{theorem}
...
\[
\mathbb{E}[\Latency^{(\zeta,\pi)}] = \Omega((\zeta T)^{1/2}), \quad \mathbb{E}[\TTFT^{(\zeta,\pi)}] = \Omega((\zeta T)^{1/2}).
\]
\end{proposition}
