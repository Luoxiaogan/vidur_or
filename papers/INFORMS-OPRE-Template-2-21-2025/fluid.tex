...
\begin{equation}
\label{eq:time_fluid}
    \text{Processing Time}^* = d_0+d_1M^* = \frac{d_0}{1-d_1\sum_{j=1}^m \lambda_j (l_j^{\prime}+1)\left(l_j + \frac{l_j^{\prime}}{2} \right)},
\end{equation}
as well as the average throughput:
\begin{align}
    \throughput^* = \frac{1}{d_0}\left(\sum_{j=1}^nn_j^*\right) \cdot \left(1- \sum_{j=1}^m \lambda_j (l_j^{\prime}+1)\left(l_j + \frac{l_j^{\prime}}{2} \right)\right) = \sum_{j=1}^m \lambda_j(l_j^{\prime}+1)\label{throughput_fluid_multiple}
\end{align}

% \arc{Should we use this for defining policy in section 4? If not we can discard this to appendix}
% Under fluid equilibrium memory capacity $M^*$, the arrival and completion ratio is:
% $$
% \frac{\text{Arrival}}{\text{Completion}}=\frac{\text{Processing Time}^*\cdot \sum_{j=1}^m \lambda_j}{ \sum_{j=1}^m \frac{n_j^*}{l_j^\prime+1}} 
% = \frac{\sum_{j=1}^m  \frac{d_0\lambda_j}{1-d_1\sum_{j=1}^m \lambda_j( l_j^{\prime}+1)\left(l_j + \frac{l_j^{\prime}}{2} \right)} }{ \sum_{j=1}^m \frac{n_j^*}{l_j^\prime+1}}=1
% $$
