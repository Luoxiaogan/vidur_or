...
In this part, we analyze the asymptotic optimality of the Nested WAIT algorithm under time-varying arrival rates, denoted $\lambda_j^t$ for prompt type $j \in [m]$ at time $t \in [0, T]$. We assume that $\lambda_j^t$ are continuous functions of time, uniformly bounded by $\lambda_{\max} < \infty$. Define the accumulated arrivals of type $j$ over the interval $[t_1, t_2]$ as $\lambda_j[t_1, t_2] := \int_{t_1}^{t_2} \lambda_j^t \, dt$. Recall from Equation \eqref{eq:wait_thresholds} that $\Delta T_{[1,2,\dots,m]}$ represents the per-iteration time cost when processing exactly $n_i$ prompts at each stage in segment $i$, for all $i \in [m]$, as \eqref{eq:nested_wait_thresholds} ($n_i$ is the threshold defined in Algorithm \ref{alg:nested_wait}).

For the Nested WAIT policy defined by $\pi = [n_1, \dots, n_m]$, we consider the following conditions for all $t \in [0, T]$ and $i = 1, 2, \dots, m-1$, analogous to \eqref{eq:nested_wait_thresholds}:
\begin{align}
    \label{eq:nested_wait_thresholds_time_varying}
    \sum_{j=1}^m \lambda_j[t, t + \Delta T_{[1,\dots,m]}(n_1,\dots,n_m)] \leq n_1, \quad 
    n_{i+1} \sum_{j=i}^m \lambda_j[t, t + \Delta T_{[1,\dots,m]}] > n_i \sum_{j=i+1}^m \lambda_j[t, t + \Delta T_{[1,\dots,m]}].
\end{align}
...
% \end{equation}
We now present the following theoretical result that extends Theorem \ref{thm:nested_wait_heavy_traffic}.
...
\[
M^{(\zeta,\pi)} \geq M^{\pi} + \sum_{j=2}^m (l + l_{j-1}') n_j + \sum_{j=2}^m (l + l_{j-1}') \theta_j^{-1} \ln\left( \frac{m \zeta T}{\delta} \right) = O\left( 2M^\pi + \sum_{j=1}^m \theta_j^{-1} (l + l_{j-1}') \ln\left( \frac{m \zeta T}{\delta} \right) \right),
\]
where the memory usage $M^\pi$ is defined as \eqref{eq:nested_wait_memory} and $\theta_j$ is given in \arc{add reference to appendix euqation}. \gan{I will add in appendix}% Replace with actual reference
...
\end{align*}
Additionally, the memory usage remains below $M^{\zeta,\pi}$ with probability at least $1 - \delta$ over $[0, T]$.
\end{theorem}
...
\begin{align}
    \label{eq:nested_wait_thresholds_L}
    \Delta T_{[1,\dots,L]}(n_1,\dots,n_L) \leq \frac{n_1}{\sum_{j=1}^L \lambda_j'}, \quad 
    \frac{n_{i+1}}{\lambda'(i+1 \to L)} > \frac{n_i}{\lambda'(i \to L)}, \quad \forall i = 1, 2, \dots, L-1,
\end{align}
where $\Delta T_{[1,\dots,L]}(n_1,\dots,n_L) = d_0 + d_1 M^\pi(n_1, \dots, n_L)$, and the  memory $M^\pi$ running with $n_i$ at each stage in segment $i$ is given by:
\[
M^{\pi}(n_1, \dots, n_L) = \sum_{i=1}^L n_i \left( l + \frac{i}{2} \Delta L\right) \Delta L.
\]

We demonstrate that the Nested WAIT algorithm (Algorithm \ref{alg:nested_wait}) with $L$ segments and thresholds $[n_1, \dots, n_L]$ satisfying Equation \eqref{eq:nested_wait_thresholds_L} achieves asymptotic optimality under the heavy-traffic conditions outlined in Section \ref{sec:nested_wait}.

\begin{theorem}
\label{thm:nested_wait_heavy_traffic, any segment}
For any fixed number of segments $L$ and thresholds $\pi = [n_1, \dots, n_L]$ satisfying Equation \eqref{eq:nested_wait_thresholds_L}, if the memory satisfies:
\begin{align}
\label{eq:nested_wait_memory_L}
M^{(\zeta, \pi)} \geq M^\pi + \sum_{j=2}^L (l + l_{j-1}') n_j + \sum_{j=2}^L (l + l_{j-1}') \theta_j^{-1} \ln\left( \frac{m \zeta T}{\delta} \right) = O\left( 2M^\pi + \sum_{j=1}^L (l + l_{j-1}') \theta_j^{-1}\ln\left( \frac{m \zeta T}{\delta} \right) \right),
\end{align}
where $\theta_j$ is given by \arc{add ref to appendix equation. Add estimation of $\theta_j$}, then the performance of Nested WAIT is guaranteed by:
\begin{align*}
&\throughput^* - \mathbb{E}\left[ \throughput^{(\zeta, \pi)} \right] = O\left( (\zeta T)^{-1} \right), \\
&\mathbb{E}\left[ \Latency^{(\zeta, \pi)} \right] = O(1), \quad \mathbb{E}\left[ \TTFT^{(\zeta, \pi)} \right] = O(1).
\end{align*}
Moreover, the memory usage remains below $M^{(\zeta, \pi)}$ with probability at least $1 - \delta$ over the time horizon $[0, T]$.
\end{theorem}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{Experiments_pdf/total_rate=50,delta = 0.001, T=20 or 2000.pdf}
