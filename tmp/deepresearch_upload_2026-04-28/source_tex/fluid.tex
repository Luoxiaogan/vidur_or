...
We now extend the balance calculation to \(m\) prompt types. Type \(j\) has input length \(l_j\), decode length \(l_j'\), and arrival rate \(\lambda_j\). Because all types draw from the same memory pool, the equilibrium is determined by their joint stage distribution and the aggregate KV-cache usage it induces.
...
\end{proposition}
The fluid model serves two purposes. First, it provides the throughput benchmark in Proposition~\ref{prop::online policy expected throughput upper bound, multiple type}. Second, the ideal-fluid equilibrium identifies the operating point a policy should maintain: roughly \(n_j^*\) type-\(j\) prompts at each stage, with total memory requirement \(M^*\). Accumulating too much late-stage work consumes memory and risks eviction, while holding too little late-stage work leaves completions below the fluid rate. Section~\ref{sec:wait} uses \(M^*\) and \(n_j^*\) to define thresholds that keep these stage populations near the ideal-fluid equilibrium.
