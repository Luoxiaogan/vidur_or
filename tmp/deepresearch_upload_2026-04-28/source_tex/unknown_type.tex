...
\begin{equation}
    ...
    &\frac{n_k}{n_{k-1}} > p_k,\forall k \in \{2, \dots, m\},
    \end{aligned}
\end{equation}
...
Assume thresholds \( \pi = [n_1, \dots, n_m] \) satisfy \eqref{eq:nested_wait_thresholds}, and memory \( M \) satisfies
\begin{equation}
    \label{eq:nested_wait_memory_total}
\begin{aligned}
    &M^{(\zeta,\pi)}\\
\geq &M^\pi + \sum_{k=2}^m (l + l_{k-1}') \bigg(n_k + \theta_k^{-1} \ln\Big( \frac{m \zeta T}{\delta(d_0+d_1M^\pi)} \Big)\bigg) \\
=& O\bigg( 2M^\pi + \sum_{k=2}^m (l + l_{k-1}') \theta_k^{-1}\ln\Big( \frac{m \zeta T}{\delta(d_0+d_1M^\pi)} \Big) \bigg),
\end{aligned}
\end{equation}
where $\theta_k \geq 8(n_k - n_{k-1}p_k)/n_{k-1}$ for \(k\ge 2\). Then:
$$
    \begin{aligned}
...
The memory condition in Theorem~\ref{thm:nested_wait_heavy_traffic} reflects the central difficulty: output lengths are not observed when prompts are admitted. As Example~\ref{ex:unknown_output} illustrates, prompts are indistinguishable at admission and reveal their output lengths only by either completing at a boundary or continuing beyond it. Completed prompts leave and release memory; surviving prompts carry their KV caches to the next segment boundary. Nested WAIT must therefore regulate admission and stage advancement while the type mix is still being revealed.

The analysis uses this same boundary structure. Conditional on a threshold-sized batch leaving segment \(k-1\), the number that continue to segment \(k\) follows binomial thinning with probability \(p_k\). This coupling turns the unknown-output-length process into boundary queues, so the proof can control the queue fluctuations that create overflow risk without tracking every resident prompt across all decode stages. In the memory condition \eqref{eq:nested_wait_memory_total}, \(M^\pi\) covers threshold-sized segment batches, and the logarithmic term is the safety buffer that keeps retained KV caches at downstream boundaries from causing overflow and eviction. The theorem states the version with one boundary per output length; Appendix~\ref{sec:extension} discusses general segment designs that group nearby output lengths into \(L\le m\) segments, and Section~\ref{sec:experiments} evaluates this implementation in simulated serving workloads.
