...
|-----|---------|----------|--------|-----|
**完整 QPS Grid (50bins, Sar256 as baseline):**

| QPS | vLLM | Sar(256) | Sar(512) | WCP | vs Sar256 | vs Sar512 |
|-----|------|----------|----------|-----|-----------|-----------|
| 10 | 1.9s | 1.7s | 1.7s | **1.6s** | **-4.1%** | **-4.1%** |
| 20 | 2.3s | 1.9s | 1.9s | **1.8s** | **-3.8%** | **-3.8%** |
| 30 | 3.2s | 2.2s | 2.2s | **2.2s** | **-3.4%** | **-3.4%** |
| 40 | 15.4s | 2.9s | 2.9s | **2.9s** | **-2.2%** | **-2.2%** |
| 50 | 30.1s | 3.9s | 4.5s | 4.4s | +11.4% | -1.6% |
| 60 | 40.0s | 12.2s | 7.5s | **8.5s** | **-29.7%** | +14.7% |
| 70 | 47.2s | 19.3s | 13.1s | **14.5s** | **-24.9%** | +10.2% |
| 80 | 52.9s | 24.5s | 18.0s | **19.8s** | **-19.2%** | +10.0% |
| 90 | 57.1s | 28.7s | 22.1s | **23.6s** | **-17.9%** | +6.9% |
| 100 | 60.2s | 32.1s | 25.3s | **27.1s** | **-15.6%** | +7.1% |
| 110 | 63.1s | 34.8s | 28.0s | **29.9s** | **-14.2%** | +6.7% |
| 120 | 65.3s | 37.1s | 30.2s | **32.2s** | **-13.2%** | +6.7% |
| 130 | 67.6s | 39.0s | 32.0s | **34.5s** | **-11.6%** | +7.7% |
| 140 | 69.1s | 40.7s | 33.7s | **36.2s** | **-11.0%** | +7.5% |
| 150 | 70.3s | 42.2s | 35.1s | **37.7s** | **-10.6%** | +7.3% |

**WCP vs Sar(256): 14/15 WIN (-2.2%~-29.7%)**. QPS=50 唯一 LOSE.
Sarathi cs/cap 对性能影响巨大: Sar(256)在 QPS=60+ 崩溃, Sar(512) 最优但需要大 batch.
