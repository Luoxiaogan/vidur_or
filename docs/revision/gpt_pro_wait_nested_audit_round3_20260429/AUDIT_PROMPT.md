# GPT Pro Third-Round Proof Audit Prompt: WAIT and Nested WAIT

You are acting as a mathematically adversarial proof auditor for an Operations Research / Management Science paper on online scheduling for LLM inference. This is a third-round audit after two prior GPT-Pro proof audits and a follow-up local `$verify-proof` pass. Your task is to determine whether the current WAIT and Nested WAIT theorem statements, algorithms, proofs, memory accounting, and scaling conventions are now mathematically correct and internally consistent.
...
6. `papers/pf_thm_timevarying.tex`: time-varying proof.
7. `papers/pf_lemmas.tex`: auxiliary lemmas.
8. `papers/model.tex`: model primitives, stage notation, memory, latency, TTFT, preemption, and eviction definitions.
9. `papers/fluid.tex`: fluid model, ideal-fluid equilibrium, and memory requirement \(M^*\).
10. `papers/extension.tex`: time-varying and general-segment extensions.
...
1. **WAIT batches all eligible types.** Algorithm 1 defines \(J(t)=\{j:n_{j0}\ge n_j\}\), batches every \(j\in J(t)\), and stores selected counts \(a_{js}\) with the batch.
...
4. **Nested WAIT prefix structure.** Algorithm 2 forms a prefix batch \(1,\dots,k\), stores selected counts, and completion advances exactly those selected prompts.
