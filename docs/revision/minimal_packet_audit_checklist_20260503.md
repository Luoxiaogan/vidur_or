...
- Items that are already covered or mostly false positives:
  ...
  - WAIT proof collapsing stages: Appendix D does not simply replace the multi-stage system by a one-dimensional queue. It builds a coupling between the event-driven WAIT policy and a conservative full-threshold comparison process. The nonstandard step is showing that this coupling controls the batch composition across prefill/decode stages and transfers entry starts to downstream stage advancement up to a negligible one-slot lag.
...
7. Overload latency and TTFT evidence.
   - Round5 concern: overload-region latency should not be described as a steady-state mean latency.
   - Current resolution: Section 6 now states that latency is averaged over a central measurement window after the initial transient, using the same window across policies and replications; effective completion-rate panels report the loss of sustainable service when a policy moves from near-overloaded to overloaded.
   - Status: Addressed for latency wording. TTFT remains primarily a theoretical metric unless additional numerical evidence is added.
