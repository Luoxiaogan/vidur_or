...
For the first segment ($k=1$), we use the same threshold-renewal convention as in Appendix~\ref{appendix::proof of nested_wait_heavy_traffic}. Idle periods before a segment-1 service start contain fewer than \(n_1\) prompts. In the time-varying setting, the no-long-lull condition in Section~\ref{sec:time_varying} is what bounds these idle periods by \(O(1)\) in service-normalized time. At renewal epochs, the state transition rule is
