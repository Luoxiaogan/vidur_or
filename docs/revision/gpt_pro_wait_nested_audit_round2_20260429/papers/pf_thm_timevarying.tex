...
\end{equation}
Since $p_k^* = \sup_b \{p_k(b)\}$ represents the worst-case thinning probability, the monotonicity of the martingale root in Appendix~\ref{appendix_lemma::proof of solution to martingale exists} yields the smallest $\theta_k$ and hence the largest memory bound term. The lower bound for $\theta_k$ follows from the same analysis in Appendix~\ref{appendix::proof of nested_wait_heavy_traffic}.
