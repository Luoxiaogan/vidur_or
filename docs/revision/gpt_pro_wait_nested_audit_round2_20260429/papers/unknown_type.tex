...
\While{True}
    ...
    \ElsIf{event is a batch completion for prefix \(1,\dots,k\)}
        ...
        \For{each stage \( s \) in segment \( k' \)}
            \State Advance the selected prompts from stage \( s \) to \( s+1 \)
        \EndFor
        \If{prompts reach boundary \( l_{k'}' \)}
            \If{\( k' < m \)}
                \State Move surviving prompts to segment \( k'+1 \)'s resident boundary queue at stage \( l_{k'}' \)
            \Else
    ...
    \If{such \( k \) exists}
        \State Form batch from prefix \(1,\dots,k\): select \( \min\{n_{k'}, Q_{k',s}\} \) prompts per stage
...
The analysis uses the same long-horizon scaling and scaled delay metrics as Section~\ref{sec:wait}: arrivals and service speed scale with \(\zeta\), and the effective horizon in the bounds is \(\zeta T\). The base threshold memory \(M^\pi\) is independent of \(\zeta\). The physical no-overflow statement, however, is a finite-horizon guarantee and requires the logarithmic safety buffer in \eqref{eq:nested_wait_memory_total}. Equivalently, for a fixed physical memory capacity, the theorem applies to horizons and failure probabilities for which this memory condition is satisfied. The new difficulty is that downstream segments receive endogenous input. The prompts entering segment \(k\) are exactly those that survived earlier segments, so their arrivals depend on upstream batching, completion events, and the resident KV caches carried across segment boundaries. We study throughput \(\throughput^{(\zeta,\pi)}\), latency \(\Latency^{(\zeta,\pi)}\), and time to first token \(\TTFT^{(\zeta,\pi)}\) under a threshold vector \(\pi=[n_1,\dots,n_m]\).
