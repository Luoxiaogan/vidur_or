...
- Use `memory consumption` for dynamic usage and `memory level` for a fixed equilibrium value.
- Describe `M^*` as the equilibrium memory level required for stability at the boundary of the stability region.
