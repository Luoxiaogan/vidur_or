...
| C4 | Equation (1) is presented as a linear memory-dependent iteration-time model with A100 validation and PD-disaggregated motivation. | `papers/model.tex` explains `d_0`, `d_1`, KV-cache-driven scope, piecewise-linear alternatives, and PD disaggregation; `papers/appendix_b_additional.tex` reports A100/SGLang calibration with `R^2=0.9943`, MAPE `1.93%`; `papers/appendix_b_additional.tex` reports PD experiment; `papers/numerical.tex` now uses the same SGLang/`1.93%` calibration wording. | Pass after manuscript sync | Updated active manuscript and response-letter wording from `affine` / unintroduced scope labels to `linear` and `multi-stage model with endogenous memory growth`. |
| C5 | The theoretical framing no longer relies on classical heavy-traffic terminology; it is asymptotic under fixed load. | `papers/known_type.tex` and `papers/unknown_type.tex` state asymptotic scaling; response letter uses "asymptotic" rather than "heavy traffic." Some theorem labels still contain legacy `heavy_traffic` identifiers, but the visible narrative has been changed. | Pass with implementation caveat | Do not mention internal label names. |
...
- Patched the organized review-comment digest so the active advisor packet refers to the reports and decision letter without committing to a referee-count convention.
...
## 2026-05-04 Real-Data Fluid-Memory Recheck

- Rechecked the response-letter and manuscript posture after adding Table~`\ref{tab:lmsys_mstar}` to Section 6.
- The new table supports the response to the \(C\ge M^*\) concern by reporting representative \(M^*(\lambda)\) values for the real dataset and comparing them with the memory capacity \(C\).
- The R2.3 response now explicitly mentions this real-data \(M^*(\lambda)\) versus \(C\) table, so the response letter and manuscript are aligned on the capacity-condition update.
- This does not change the conservative real-data provenance posture: the paper still reports calibration rules, finite grids, and plotted-rate behavior, while exact per-arrival-rate winning configurations for the full lmsys Figure D sweep remain an open rerun/provenance item.
